...
int main(){
    ...
    while(opcao != 3){
        ...
        switch(opcao){
            ...
            case 1:
                recvfrom(socketCliente, fileName, strlen(fileName), 0, (struct sockaddr *)&cliaddr2, sizeof(struct sockaddr));
    ...
    /*
    	//Adicionando informações do segundo servidor
	cliaddr2.sin_family = AF_INET;
	cliaddr2.sin_port = htons(8000);
	cliaddr2.sin_addr.s_addr = IP;

	sendto(socketClient, fileName, strlen(fileName), 0, (struct sockaddr *)&cliaddr2, sizeof(struct sockaddr));

	recvfrom(socketClient, file_buffer
...
*/
    ...
    if(sendto(sd, file_buffer, strlen(file_buffer), 0,(struct sockaddr *)&servaddr, sizeof(struct sockaddr))<0){
